...
for elem_ind in range(num_elem):
    ...
    if num_ptr_collapse >= 1:  # this is a crack aligned element
        ...
        if (
            len(np.argwhere(coord_data_x > start_ptr)) >= 1
            and len(np.argwhere(coord_data_x < end_ptr)) >= 1
        ):  # hardcode #within fault region
...
for elem_ind in range(num_elem):
    ...
    if num_ptr_collapse >= 1:  # this is a crack aligned element
        ...
        coord_data_y = m.points[elem_connect_i][:, 1]  # numpy.ndarray
        if (
            len(np.argwhere(slope_prep * coord_data_x + b_start_30 - coord_data_y >= 0))
            > 1
            and len(
                np.argwhere(slope_prep * coord_data_x + b_end_30 - coord_data_y <= 0)
            )
            > 1
        ):  # slope_prep > 0
            if (
                slope * np.sum(coord_data_x) / 3 + b_30 - np.sum(coord_data_y) / 3 < 0
